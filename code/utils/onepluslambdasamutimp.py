...
class OnePlusLambdaSAMutImp:
    def __init__(self, problem, problem_size, mutation_probability, 
                 offspring_size, not_used_1, mut_update_factor, not_used_3):
    ...
    def __next__(self):
        ...
        for i in range(math.ceil(self.offspring_size/2)):
            mutated_string = self.mutate(self.mutation_probability
                                         /self.mut_update_factor)
            offspring.append([self.problem.evaluate(mutated_string), 
                              mutated_string, 1])
            self.evaluations += 1
        for i in range(math.ceil(self.offspring_size/2)):
            mutated_string = self.mutate(self.mut_update_factor
