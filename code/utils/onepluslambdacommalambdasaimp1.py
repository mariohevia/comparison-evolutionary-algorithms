...
class OnePlusLambdaCommaLambdaSAImp1:
    def __init__(self, problem, problem_size, not_used_0, 
        ...
        self.offspring_size = max(offspring_size, 3)
...
#        print('\n'+self.bit_string, self.problem.evaluate(self.bit_string)[0])
        ...
        if l == 0:
            l = 1
        for i in range(math.floor(round(self.offspring_size)*0.37)):
            mutated_string = self.mutate(l)
            offspring.append((self.problem.evaluate(mutated_string), 
                              mutated_string))
            self.evaluations += 1
        fittest_mut37 = self.select_1(offspring)
        for i in range(math.ceil(round(self.offspring_size)*0.63)):
            mutated_string = self.mutate(l)
            mutated_fittness = self.problem.evaluate(mutated_string)
            self.evaluations += 1
            if mutated_fittness[0] > fittest_mut37[0][0]:
                offspring = [(mutated_fittness, mutated_string)]
                fittest_mut = (mutated_fittness, mutated_string)
                break
            else:
                fittest_mut = fittest_mut37
#        offspring = [] # ask if comment or not
        for i in range(round(self.offspring_size)):
            cross_string = self.crossover(fittest_mut[1])
...
#              self.evaluations, len(fittest_offspring))
    ...
    def update(self):
        ...
        self.offspring_size = max(self.offspring_size, 3)
