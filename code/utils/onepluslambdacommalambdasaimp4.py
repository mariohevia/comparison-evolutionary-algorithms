...
class OnePlusLambdaCommaLambdaSAImp4:
    def __init__(self, problem, problem_size, mutation_probability, 
                 offspring_size, cross_prob, not_used_2, 
...
#        print(self.generations, '\n' + self.bit_string, '\n' + ''.join(mutated_list), '\n')
    ...
    def calc_difference(self, bit_string_1, bit_string_2):
        difference = [str(xor(int(b), int(m))) for b, m in 
                      zip(bit_string_1, bit_string_2)].count('1')
        return difference
    ...
    def select_1(self, offspring):
        ...
        list_dif = [(child, self.calc_difference(child, self.parent[1])) for child in fittest_offspring]
        eval_fittest = heapq.nlargest(1, list_dif)[0][1]
        fittest_offspring = [child[0] for child in list_dif if 
                             child[1] == eval_fittest]
        # fittest = bit_string
